...
def get_latest_color():
    ...
    try:
        ...
        if row:
            color = row['color']
            conn.execute("UPDATE color_commands SET processed = 1 WHERE id = ?", (row['id'],))
            conn.commit()
            return color
        return None
    except Exception as e:
        print(f"خطا در دریافت رنگ: {e}")
        return None
    finally:
        conn.close()
...
def get_latest_action():
    ...
    try:
        ...
        if row:
            action = row['action']
            conn.execute("UPDATE action_commands SET processed = 1 WHERE id = ?", (row['id'],))
            conn.commit()
            return action
        return None
    except Exception as e:
        print(f"خطا در دریافت اقدام: {e}")
        return None
    finally:
        conn.close()
...
color_bar_height = 40
colors = {
    "red": {"bgr": (0, 0, 255), "name": "قرمز", "text_color": (200, 0, 0), "border_color": (200, 0, 0)},
    "green": {"bgr": (0, 255, 0), "name": "سبز", "text_color": (0, 180, 0), "border_color": (0, 180, 0)},
    "blue": {"bgr": (255, 0, 0), "name": "آبی", "text_color": (0, 0, 200), "border_color": (0, 0, 200)},
    "yellow": {"bgr": (0, 255, 255), "name": "زرد", "text_color": (180, 180, 0), "border_color": (180, 180, 0)},
    "eraser": {"bgr": (0, 0, 0), "name": "پاکن", "text_color": (255, 255, 255), "border_color": (200, 0, 0)}
}
...
for color_info in colors.values():
    color_bar[:, x_offset:x_offset+color_width] = color_info["bgr"]
...
while cap.isOpened():
    ...
    # پردازش دستورات دیتابیس
    latest_color = get_latest_color()
    if latest_color and latest_color in colors:
        selected_color = colors[latest_color]["bgr"]
        coloring_enabled = True
        if selected_color != animation_states["color"]["last_color"]:
    ...
    if latest_action and latest_action in operations:
        if latest_action == "undo" and current_time - last_undo_time > gesture_debounce:
            if undo_stack:
        ...
        elif latest_action == "reset" and current_time - last_reset_time > gesture_debounce:
        ...
        elif latest_action == "next" and current_time - last_next_time > gesture_debounce:
        ...
        elif latest_action == "prev" and current_time - last_prev_time > gesture_debounce:
        ...
        elif latest_action == "save" and current_time - last_save_time > gesture_debounce:
            if save_image():
                save_active = True
    ...
    if hands_results.multi_hand_landmarks and hands_results.multi_handedness:
        for hand_landmarks, handedness in zip(hands_results.multi_hand_landmarks, hands_results.multi_handedness):
            ...
            if selected_color:
                color_info = next((info for info in colors.values() if info["bgr"] == selected_color), 
                                 {"name": "ناشناخته", "text_color": (255, 255, 255), "border_color": None})
